...
namespace RobotKit.Internal
{
    public class RobotSession
    {
        ...
        private RfcommDeviceService _peerInfo;
        ...
        public RobotSession(RfcommDeviceService peerInfo, Robot robot, CoreDispatcher coreDispatcher)
        {
            _peerInfo = peerInfo;
            parser = new ResponseParser(robot);
        ...
        private async void DispatchResponseThreadSafe(Response response)
        {
            if (_responseDelegate != null)
            {
                var coreDispatcher = _coreDispatcher;
                await coreDispatcher.RunAsync(CoreDispatcherPriority.Normal, () => _responseDelegate(response));
            }
        }
        ...
        public async Task<Boolean> Initialize()
        {
            ...
            try
            {
                await _socket.ConnectAsync(_peerInfo.ConnectionHostName, "1");
               PostSocketRead(16);
                flag = true;
                return flag;
            }
